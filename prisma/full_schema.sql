-- RegLens Full Database Schema (MySQL)
-- This script creates the entire database schema used by the application.
-- It is equivalent to applying all Prisma migrations.

-- 1) Create database (if it does not exist yet)
...
-- 2) Core schema (initial migration)
...
-- 3) Notifications (second migration)
